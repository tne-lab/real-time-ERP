...
void ERPVisualizer::updateElectrodeButtons(int numInputs, int numButtons)
{
	juce::Rectangle<int> bounds;
...
void ERPVisualizer::updateCombList()
{
...
void ERPVisualizer::updateGroupState()
{
...
void ERPVisualizer::labelTextChanged(Label* labelThatHasChanged)
{
	...
	if (labelThatHasChanged == fstartEditable)
	{
		...
		{
            processor->setParameter(ProcessorPlugin::START_FREQ, static_cast<int>(newVal));
		}
	}
	if (labelThatHasChanged == fendEditable)
	{
		...
		if (updateIntLabel(labelThatHasChanged, 0, INT_MAX, 8, &newVal))
		{
            processor->setParameter(ProcessorPlugin::END_FREQ, static_cast<int>(newVal));
		}
	}
}
...
void ERPVisualizer::createElectrodeButton(int chan)
{
	int xPos = 15;
	juce::Rectangle<int> bounds;
...
bool ERPVisualizer::updateFloatLabel(Label* label, float min, float max,
...
bool ERPVisualizer::updateIntLabel(Label* label, int min, int max, int defaultValue, int* out)
{
	const String& in = label->getText();
	int parsedInt;
	try
...
void ERPVisualizer::UpdateElectrodeOnTransition()
{
    //int numInputs = processor->getNumInputs();
